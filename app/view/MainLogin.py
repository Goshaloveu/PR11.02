...
class MainLoginWindow(Window):
    ...
    def exit_login(self):
        self.flag = "login"
        self.type = self.login.ComboBox.text()
    ...
    def exit_reg(self):
        ...
        self.number = re.sub(r"[^0-9+]", "", self.registration.lineEdit_3.text())[-10:]
...
class RegWindow(QWidget, Ui_Form_Registration):
    ...
    def enter(self):
        ...
        if not self.number:
            ...
            if bool(re.fullmatch(r"^(?:\+7|8)\d{10}$", self.number)):
                check_availale = None
                if self.type == "Соц. Работник":
                    check_availale = rq.get_owner(self.number[-10:])
                else:
                    check_availale = rq.get_client(self.number[-10:])
                
                if check_availale:
                    self.mail_error = Flyout.create(
                        icon=InfoBarIcon.ERROR,
                        title='Ошибка регистрации!',
                        content=self.tr('Аккаунт с этим номером уже зарегестрирован!'),
                        target=self.lineEdit_3,
                        parent=self,
                        isClosable=True
                    )
        ...
        if not self.password:
            ...
            if not bool(re.fullmatch(r"[a-z0-9@_-]*", self.password)):
                self.password_error = Flyout.create(
                    icon=InfoBarIcon.WARNING,
                    title='Некорректный пароль!',
    ...
    def enter(self):
        ...
        if not self.number:
            ...
            if bool(re.fullmatch(r"^(?:\+7|8)\d{10}$", self.number)):
                ...
                if not check_availale:
                    self.mail_error = Flyout.create(
                        icon=InfoBarIcon.ERROR,
                        title='Ошибка входа!',
                        content=self.tr('Такого аккаунта не существует!'),
                        target=self.lineEdit_3,
                        parent=self,
                        isClosable=True
                    )
                else:
                    if self.password and (check_availale.password == self.password):
                        # print(check_availale.password == self.password)
                        self.isPasswordValid = True
                    elif self.password:
                        self.mail_error = Flyout.create(
                            icon=InfoBarIcon.ERROR,
                            title='Ошибка входа!',
                            content=self.tr('Неправильный пароль!'),
                            target=self.lineEdit_4,
                            parent=self,
                            isClosable=True
                        )
